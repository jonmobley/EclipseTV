...
/// Drives the companion's MultipeerConnectivity link to one or more Apple TVs: discovery,
/// invitations, the active/replica peer model, and library-control command sending.
///
/// This core file owns connection lifecycle and command sending. The remaining behavior is
/// split across same-type extensions to keep each file focused:
/// - `+Browser`: `MCNearbyServiceBrowserDelegate` (peer discovery).
/// - `+Session`: `MCSessionDelegate` (state changes, inbound control routing).
/// - `+Retry`: exponential-backoff reconnect to the active peer.
/// - `+Transfers`: user-initiated media sends with progress reporting.
///
/// Several members below are `internal` rather than `private` specifically so those
/// extension files can reach them; treat them as file-cluster-private.
class iPhoneConnectionManager: NSObject {
    ...
    let logger = Logger(subsystem: "com.eclipseapp.ios", category: "ConnectionManager")
    ...
    /// this peer. Read-only outside the type; mutate only via `setActivePeer(_:)` (the
    ...
    // Keep track of AppleTV's move mode state. Read-only outside the type; mutate only via
    // `setAppleTVInMoveMode(_:)` (driven by the `+Session` extension).
    private(set) var isAppleTVInMoveMode = false
    
    // Connection retry mechanism (driven by the `+Retry` extension).
    var retryCount = 0
    let maxRetries = 3
    ...
    func clearPendingInvite(for peer: MCPeerID) {
    ...
    /// The sanctioned mutator for `activePeer`. Kept so the property stays read-only to
    /// callers outside the type while the connection-state handling lives in `+Session`.
    func setActivePeer(_ peer: MCPeerID?) {
        activePeer = peer
    }

    /// The sanctioned mutator for `isAppleTVInMoveMode` (set from `+Session`).
    func setAppleTVInMoveMode(_ enabled: Bool) {
        isAppleTVInMoveMode = enabled
    }
